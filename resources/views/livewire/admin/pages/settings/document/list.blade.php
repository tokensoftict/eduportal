            ...
            <div class="card card-secondary">
                ...
                <div class="card-body">
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody>
                        @php($count=0)
                        @foreach(\App\Models\DocumentUpload::all() as $gender)
                            <tr>
                                <td>{{++$count}}</td>
                                <td>{{$gender->name}}</td>
                                ...
                                <td>
                                    <div class="btn-group">
                                        <a href="{{ route('document.edit',$gender->id)}}" class="btn btn-primary">Edit</a>
                                        <form action="{{ route('document.destroy', $gender->id)}}" method="post">&nbsp;&nbsp;&nbsp;
                                            @csrf
                                            @method('DELETE')
